  ...
  v-dialog(v-model='referral_link', dark, max-width='500')
    v-card(style='background-color: #333')
      ...
      v-card-text
        ...
        p(v-if='target')
          a(
            :href='"mailto:" + target + "?subject=CoinOS Referral&body=<a href=" + referral_link + "> Click here to activate your referral </a>"'
          ) Link
      v-card-actions
...
export default {
  ...
  methods: {
    ...
    async generateLink() {
      ...
      var token =
        this.status === 'New' ? this.tokens[this.tokens.length - 1].token : '';

      console.log('Retrieved ' + token);
      this.referral_link =
        config.servers[process.env.NODE_ENV] ||
        'https://coinos.io/funding/referral/' + token;
    ...
    async generateQR(version) {
      ...
      if (!version) {
        version = 8;
      } // sizes range from 1 - 40 indicating size / capacity
